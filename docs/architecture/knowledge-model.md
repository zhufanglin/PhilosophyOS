...
PhilosophyOS 支持三种模式，但它们共享同一套 `user_id` / `workspace_id` 数据边界：

| 模式 | 是否需要登录 | 数据位置 | 同步能力 |
| --- | --- | --- | --- |
| 本地模式 | 否 | 本机 SQLite、用户指定的 Obsidian 目录 | 不上传，单设备使用 |
| 云同步模式 | 是 | 本地缓存 + PhilosophyOS 云端 | 同一用户的设备同步 |
| 团队工作区模式 | 是 | 工作区云端数据 + 本地缓存 | 按 membership 和 role 共享 |

本地模式必须是默认入口。用户不提供账号时，系统使用固定的本地用户和本地工作区；这不是匿名云账号，也不会自动把内容上传到服务器。

### 11.2 登录与切换规则

账户切换只改变“当前身份和当前工作区”，不直接搬运或覆盖数据。

1. 启动时先恢复本地工作区，不要求登录。
2. 用户登录后，创建或选择一个云端工作区。
3. 系统显示本地工作区与云端工作区的对象数量、最近更新时间和冲突数量。
4. 用户明确选择“上传本地内容”“下载云端内容”或“保持分开”。
5. 同步完成后才切换当前工作区；失败时保留原工作区，不进入半同步状态。
6. 退出登录时回到本地工作区或只读缓存，不删除本地数据。

浏览器传入的 `workspace_id` 只能表达选择意图。后端必须从认证身份和 `workspace_memberships` 推导可访问工作区，再对所有读写查询强制添加 `workspace_id` 过滤。

### 11.3 同步边界

同步对象必须带有工作区归属、版本号和更新时间。首版只同步用户私有对象：

- 对话会话与消息；
- 思想快照、观点卡和开放问题；
- 用户创建的 Markdown 草稿及写入审计；
- 工作区级模型配置的非敏感元数据。

公共哲学家、概念、来源和关系不复制到用户工作区；用户对公共对象的收藏、筛选和关联属于自己的工作区。

API Key 不参与云同步。用户切换设备后需要重新配置本地 Key，或由未来的托管密钥服务提供工作区级密钥引用。任何导出、日志和同步包都不得包含完整 Key 或加密后的 Key envelope。

### 11.4 冲突与删除

- 同一对象在两个设备修改时，保留两个版本并标记冲突，不静默覆盖。
- 对话和快照采用追加事件优先；用户确认后的观点卡采用显式版本合并。
- Obsidian 写入必须继续遵循 diff 预览、用户确认、哈希校验和可撤销审计。
- 云端删除优先使用 soft delete；本地缓存收到删除事件后先标记，再由用户确认清理。
- 删除工作区是 owner-only 操作，必须展示对象数量、导出入口和不可逆影响。
